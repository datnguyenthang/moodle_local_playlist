...
//require_once($CFG->dirroot.'/local/playlist/lib/lib.php');
...
class video_edit_form extends moodleform {
    ...
    function definition() {
        ...
        */
        ...
        $mform->addElement('html', '<div class="text-center"><strong>OR</strong></div>' );
        
        $mform->addElement('filemanager', 'file_filemanager', local_playlist_get_string('files'), null, $this->_customdata['filevideooptions']);
        //$mform->addRule('file_filemanager', get_string('requiredelement', 'form'), 'required', null, 'server');
    ...
    function validation($data, $files) {
        ...
        if (empty($data['link']) && empty($files['file_filemanager']) ) {
           $errors['link'] = get_string('requirelinkorvideo', 'local_playlist');
        }
        if (!filter_var($data['link'], FILTER_VALIDATE_URL)) { 
            $errors['link'] = get_string('invalidurl', 'local_playlist');
        }
